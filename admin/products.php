	...
	switch ($_GET['action']) {
		case 'delete':
			...
			foreach ($GaP as $key => $value) {
				unlink('../images/products/' . $value['image_name']);
			}
...
<div class="content">
	...
	<table>
		...
		<tr>
			<td><?=$value['title']?></td>
			<?php 
			if($value['main_image'] != '' && file_exists('../images/products/'.$value['main_image'])) { 
				echo '<td>
						<img src="../images/products/'.$value['main_image'].'" alt="" width="100">
					</td>';
			} else {
